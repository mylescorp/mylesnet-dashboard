const credentialPrefix = "mnrc.v1";
...
async function encryptionKey(): Promise<CryptoKey> {
  const encodedKey = process.env.ROUTER_CREDENTIALS_ENCRYPTION_KEY;
  if (!encodedKey) {
    throw new Error("Router credential protection is not configured.");
  }
...
export async function encryptRouterCredential(value: string): Promise<string> {
 ...
 * preserve existing router connectivity until the owner runs the migration.
 */
export async function decryptRouterCredential(value: string): Promise<string> {
  ...
  if (!encodedIv || !encodedCipherText) {
    throw new Error("Router credential data is invalid.");
  }
  ...
  try {
    ...
    throw new Error("Router credentials could not be read.");
  }
}

export function isEncryptedRouterCredential(value: string): boolean {
