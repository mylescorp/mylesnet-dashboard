  ...
  return (
    <div className="workspace-page">
      <p className="pf-badge">
        <Link href="/platform/markets">← All markets</Link>
      </p>
      ...
      <div className="pf-stack">
        {staffing.isUnstaffed && market.lifecycleStatus === "active" && (
          <div className="pf-panel platform-action-card-platform-warning" style={{ borderColor: "color-mix(in srgb, var(--warning) 40%, var(--line))" }}>
            ...
            <Link href={`/platform/agents?market=${marketId}`} className="primary-button" style={{ textDecoration: "none", display: "inline-block", marginTop: 8 }}>
              Assign an agent
            </Link>
          </div>
        )}
              ...
              <table className="pf-table">
                ...
                <tbody>
                  {staffing.agents.map((a) => (
                    <tr key={a.agentId}>
                      <td><Link href={`/platform/agents/${a.agentId}`}>{a.agentName}</Link></td>
