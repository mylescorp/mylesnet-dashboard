...
export default function AccessManagementPage() {
  ...
  const migrateRouterCredentials = async () => {
    setMigratingCredentials(true);
    try {
      const result = await migrateCredentials({});
      setMessage(result.migrated ? `Protected ${result.migrated} router credential record${result.migrated === 1 ? "" : "s"}.` : "All router credential records are already protected.");
    } catch {
      setMessage("We could not protect the router credential records. Please try again.");
    } finally {
