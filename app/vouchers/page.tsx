...
function BatchRow({
  ...
  batchId: Id<"voucherBatches">;
  batch: Doc<"voucherBatches">;
  markets: Doc<"markets">[];
  agents: Doc<"agents">[];
}) {
  const [open, setOpen] = useState(false);
  const market = markets.find((m) => m._id === batch.marketId);
  const rowOpen = open;
...
function BatchVouchers({ batchId, agents }: { batchId: string; agents: Doc<"agents">[] }) {
    ...
    <div>
      ...
      <div className="pf-table-wrap">
        <table className="pf-table">
          ...
          <tbody>
            {vouchers.map((v) => {
              const owner = agents.find((a) => a._id === v.ownerAgentId);
              return (
                <tr key={v._id}>
