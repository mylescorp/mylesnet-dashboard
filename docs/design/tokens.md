...
- **Theme**: the storage scheme is now tri-state (`light` / `dark` / `system`, key `mylesnet-dashboard-theme`). The no-flash script in the root layout resolves `system` at pre-paint; `apps/web/shared/components/ThemeToggle.tsx` cycles light → dark → system and follows live OS changes in system mode.
