...
| Repo folder | `apps/web/features/captive-portal/` (feature slice) + `docs/captive-portal/` (spec + flow mirrors) |
...
feature folder `apps/web/features/captive-portal/` so portal code stays beside
the web product it serves.

### 2.2 Repository folder contract

```text
apps/web/features/captive-portal/
...
|---|---|---|
| All logic, UI, templates, i18n, config, tests, ADRs | `apps/web/features/captive-portal/` | web-local feature home |
...
| Convex **adapters** (thin query/mutation/action/httpAction glue calling `apps/web/features/captive-portal/core`) | `convex/portal/` | Convex reads functions from a single function root; adapters hold only glue |
| Schema tables (central) | `convex/schema.ts` + schema additions (see §18) | Convex indexes schema centrally |
| Path alias `@portal/*` → `apps/web/features/captive-portal/*` | `apps/web/tsconfig.json` + `apps/web/next.config.ts` | clean imports from stubs and adapters |

**Rule of thumb:** logic is added in `apps/web/features/captive-portal/`, never in `app/` or `convex/`.
Those two only ever gain re-export stubs and thin adapters. A `apps/web/features/captive-portal/CHANGELOG.md`
...
graph TD
    ...
    subgraph Logic_Layer ["Application Logic - Convex"]
        ...
        Core[apps/web/features/captive-portal/core domain logic]
...
- Repository: `apps/web/features/captive-portal/` (code), `docs/captive-portal/` (mirrors)
