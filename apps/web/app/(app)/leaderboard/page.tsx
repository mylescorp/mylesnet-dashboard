...
export default function LeaderboardPage() {
  ...
  const rankIcon = (rank: number) => {
    if (rank === 1) return <Trophy size={18} color={designToken.rank.gold} />;
    if (rank === 2) return <Medal size={18} color={designToken.rank.silver} />;
    if (rank === 3) return <Award size={18} color={designToken.rank.bronze} />;
