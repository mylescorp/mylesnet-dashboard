    ...
    <div className="workspace-page tenant-control-page">
      ...
      <section className="pf-panel">
        ...
        {tenants === undefined ? <p className="pf-muted">Loading tenant inventory…</p> : tenants.length === 0 ? <EmptyTenantState onRegister={() => setCreating(true)} /> : (
          <div className="pf-table-wrap"><table className="pf-table"><thead><tr><th>Tenant</th><th>Identity</th><th>Members</th><th>Markets</th><th>Subscribers</th><th>Plan</th><th>Status</th><th /></tr></thead><tbody>
            {tenants.map((tenant) => <tr key={tenant._id}><td><Link href={`/platform/tenants/${tenant._id}`} className="tenant-name-link"><strong>{tenant.name}</strong></Link><small className="table-subtext">{tenant.slug} · {tenant.country}{tenant.accountOwner ? <><br />Owner: {tenant.accountOwner.name ?? tenant.accountOwner.email}</> : null}</small></td><td>{tenant.workosOrganizationId ? <span className="tenant-linked"><Link2 size={14} aria-hidden="true" />Connected</span> : <span className="tenant-unlinked"><CircleAlert size={14} aria-hidden="true" />Missing mapping</span>}</td><td>{tenant.membershipCount}</td><td>{tenant.marketCount}</td><td>{tenant.subscriberCount}</td><td>{tenant.entitlement ? `${tenant.entitlement.planId} · ${tenant.entitlement.status}` : "Not configured"}</td><td><Status status={tenant.status} /></td><td><div className="cell-actions">{tenant.status === "suspended" ? <button type="button" className="secondary-button" disabled={workingId === tenant._id} onClick={() => void changeStatus(tenant, "active")}><PlayCircle size={14} aria-hidden="true" />Activate</button> : <button type="button" className="secondary-button" disabled={workingId === tenant._id || tenant.status === "cancelled"} onClick={() => void changeStatus(tenant, "suspended")}><PauseCircle size={14} aria-hidden="true" />Suspend</button>}</div></td></tr>)}
          </tbody></table></div>
        )}
      </section>
